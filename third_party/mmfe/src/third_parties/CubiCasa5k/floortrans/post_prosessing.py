...
def extract_wall_polygon(wall, wall_points, segmentation, seg_class):
    ...
    if w_dim == 1:
        ...
        # Handle different return types from stats.mode() across different scipy versions
        if hasattr(mode_result, 'mode'):
            mode_value = mode_result.mode
            if hasattr(mode_value, '__getitem__') and hasattr(mode_value, '__len__') and len(mode_value) > 0:
                wall_width = mode_value[0]
            else:
                wall_width = mode_value
        else:
            # Direct return (newer scipy versions)
            if hasattr(mode_result, '__getitem__') and hasattr(mode_result, '__len__') and len(mode_result) > 0:
                wall_width = mode_result[0]
            else:
                wall_width = mode_result
        if wall_width > y2 - y1:
            wall_width = y2 - y1
