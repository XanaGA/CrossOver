...
class House:
    def __init__(self, path, height, width, icon_list=icons_selected, room_list=rooms_selected):
        ...
        timings["parse_svg"] = time.perf_counter() - t0
        ...
        for e in svg.getElementsByTagName('g'):
            try: 
                ...
                if e.getAttribute("id") == "Railing":
                    ...
                    wall_id += 1
                    t_railing += time.perf_counter() - t1
            ...
            # (commented for efficiency, NO WINDOW LABELS)
            ...
            # (commented for efficiency, NO DOOR LABELS)
            ...
            if "FixedFurniture " in e.getAttribute("class"):
                ...
                t_furniture += time.perf_counter() - t1

            # (commented for efficiency, NO ROOM LABELS)
            ...
            #     t_space += time.perf_counter() - t1

            # (commented for efficiency, NO STAIRS LABELS)
        ...
        timings["get_avg_wall_width"] = time.perf_counter() - t0

        t0 = time.perf_counter()
        self.new_walls = self.connect_walls(self.wall_objs)
        timings["connect_walls"] = time.perf_counter() - t0

        t0 = time.perf_counter()
        for w in self.new_walls:
            w.change_end_points()
        timings["update_end_points"] = time.perf_counter() - t0

        t0 = time.perf_counter()
        for w in self.pillar_walls:
            self.new_walls.append(w)
        timings["append_pillars"] = time.perf_counter() - t0

        t0 = time.perf_counter()
        self.points = self.lines_to_points(
            self.width, self.height, self.new_walls, self.avg_wall_width)
        timings["lines_to_points"] = time.perf_counter() - t0
        t0 = time.perf_counter()
        self.points = self.merge_joints(self.points, self.avg_wall_width)
        timings["merge_joints"] = time.perf_counter() - t0

        # # walls to representation (commented for efficiency, don't know the implications)
        # t0 = time.perf_counter()
        # for w in self.new_walls:
        ...
        # timings["build_representation"] = time.perf_counter() - t0

        # aggregate per-element timings
        timings["handle_walls"] = t_walls
        timings["handle_railing"] = t_railing
        timings["handle_windows"] = t_window
        timings["handle_doors"] = t_door
        timings["handle_furniture"] = t_furniture
        timings["handle_spaces"] = t_space

        timings["total"] = time.perf_counter() - start_total

        # # pretty print summary in milliseconds
        # print("\n--- House.__init__ timings (ms) ---")
        # # Show total first
        # print(f"total: {timings['total']*1000:.2f}")
        # for k in [
        #     "parse_svg",
        #     "handle_walls",
        #     "handle_railing",
