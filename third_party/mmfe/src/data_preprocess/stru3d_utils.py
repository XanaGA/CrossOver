...
def generate_density(point_cloud, width=256, height=256):
    ...
    max_coords = max_coords + 0.01 * max_m_min
    min_coords = min_coords - 0.01 * max_m_min

    # Compute aspect ratio in 2D for the pointcloud
    aspect_ratio = (max_coords[1]-min_coords[1])/(max_coords[0]-min_coords[0])

    image_res = np.array((width, height))
    image_res[1] *= aspect_ratio
    ...
    density = np.zeros((image_res[1], image_res[0]), dtype=np.float32)
