...
from transformations import Transformation2D, Transformation3D, TransformationSpherical
...
def load_floor_plan_image(floorplan_image_file_name: str) -> np.ndarray:
    """Load a floor plan image and normalize it to RGB with a white background."""
    png_floor_image = cv2.imread(floorplan_image_file_name, cv2.IMREAD_UNCHANGED)
    if png_floor_image is None:
        raise FileNotFoundError(
            "Unable to load floor plan image: {}".format(floorplan_image_file_name)
        )

    if png_floor_image.ndim == 2:
        png_floor_image = cv2.cvtColor(png_floor_image, cv2.COLOR_GRAY2BGR)

    if png_floor_image.shape[2] == 4:
        bgr = png_floor_image[:, :, :3].astype(np.float32)
        alpha = png_floor_image[:, :, 3].astype(np.float32) / 255.0
        white_background = np.full_like(bgr, 255.0)
        bgr = bgr * alpha[..., None] + white_background * (1.0 - alpha[..., None])
        png_floor_image = bgr.astype(np.uint8)
    else:
        png_floor_image = png_floor_image[:, :, :3]

    return cv2.cvtColor(png_floor_image, cv2.COLOR_BGR2RGB)


def draw_transformed_geometry(
    ...
    *,
    ...
    """Draw a list of polygons on top of a base image after applying the given transformation."""
    canvas = base_image.copy()

    for room_wdo_poly in room_wdo_poly_list:
        if room_wdo_poly.num_points == 0:
            continue

        if room_wdo_poly.type == PolygonType.ROOM:
            color = (255, 0, 0)
        else:
            color = POLYGON_COLOR.get(room_wdo_poly.type)

        if color is None:
            # Skip unsupported polygon types like cameras or pin labels.
            continue

        polygon_coords = np.array(
            [(point.x, point.y) for point in room_wdo_poly.points], dtype=np.float32
        )
        polygon_coords = transformation.apply_inverse(polygon_coords)
...
def save_rgb_image(image_rgb: np.ndarray, output_file_name: str):
    """Save an RGB image to disk."""
    cv2.imwrite(output_file_name, cv2.cvtColor(image_rgb, cv2.COLOR_RGB2BGR))
...
def render_raster_to_vector_alignment(
    ...
    :return: None
    """
    base_image_rgb = load_floor_plan_image(floorplan_image_file_name)
    rendered_image = draw_transformed_geometry(
        base_image_rgb, room_wdo_poly_list, transformation, thickness=2
    )
    save_rgb_image(rendered_image, output_file_name)
