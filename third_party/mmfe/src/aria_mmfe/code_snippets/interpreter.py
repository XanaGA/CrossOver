...
import numpy as np
from .constants import UNIT_CUBE_VERTICES
...
def language_to_bboxes(entities):
    ...
    for command, params in entities:
        ...
        # accumulate world vertices for global AABB computation
    ...
    # Compute an axis-aligned bounding box that encapsulates all boxes
    scene_aabb = None
    if len(all_verts) > 0:
        all_verts_np = np.vstack(all_verts)
        vmin = np.min(all_verts_np, axis=0)
        vmax = np.max(all_verts_np, axis=0)
        scene_aabb = {"min": vmin, "max": vmax}
    return box_definitions, scene_aabb
