...
parser.add_argument("--unzip", action="store_true", help="Unzip the downloaded zip files. ", required=True)
    ...
    for i, chunk_id in enumerate(chunk_ids_to_download):
        ...
        with tqdm(
            ...
            if unzip_flag:
                ...
                # Post-unzip cleanup: remove heavy/unneeded assets
                targets_dirs = {"depth", "instances", "rgb"}
                target_file = "semidense_observations.csv.gz"

                for walk_root, dirnames, filenames in os.walk(output_dir):
                    # Remove specified directories wherever they appear
                    for d in list(dirnames):
                        if d in targets_dirs:
                            dir_path = os.path.join(walk_root, d)
                            try:
                                shutil.rmtree(dir_path)
                                print(f"Removed directory {dir_path}")
                            except Exception as e:
                                print(f"Warning: failed to remove directory {dir_path}: {e}")
                            # Prevent walking into a directory that is gone
                            try:
                                dirnames.remove(d)
                            except ValueError:
                                pass

                    # Remove the specified file wherever it appears
                    if target_file in filenames:
                        file_path = os.path.join(walk_root, target_file)
                        try:
                            os.remove(file_path)
