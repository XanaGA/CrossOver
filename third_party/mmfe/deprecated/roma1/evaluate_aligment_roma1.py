...
def save_best_worst_alignments(batch, batch_results, batch_idx: int, cfg: DictConfig):
    ...
    fig_path = os.path.join(out_dir, f'batch_{batch_idx:04d}.png')
    ...
    for row_idx, (res, prefix) in enumerate(zip(picks, prefixes)):
        ...
        if corners_original is not None and len(corners_original) > 0:
            for corner_idx, (corner, label) in enumerate(zip(corners_original, corner_labels)):
                ...
                axs[row_idx, 0].text(corner[0] + 5, corner[1] - 5, label, color='white', 
        ...
        if corners_gt is not None and len(corners_gt) > 0:
            for corner_idx, (corner, label) in enumerate(zip(corners_gt, corner_labels)):
                axs[row_idx, 1].scatter(corner[0], corner[1], color='green', s=80, marker='o',
                                       edgecolors='white', linewidth=2, alpha=0.8)
                axs[row_idx, 1].text(corner[0] + 5, corner[1] - 5, f'{label}_GT', color='white',
                                    fontsize=8, fontweight='bold',
                                    bbox=dict(boxstyle='round,pad=0.2', facecolor='green', alpha=0.7))

        accuracy = res.get('accuracy', 0)
        axs[row_idx, 2].imshow(corrected)
        axs[row_idx, 2].set_title(f'{prefix} - Corrected (Acc: {accuracy:.1f}%)')
        ...
        if corners_gt is not None and corners_pred is not None and len(corners_gt) > 0 and len(corners_pred) > 0:
            ...
            for corner_idx, (corner, label) in enumerate(zip(corners_original, corner_labels)):
                axs[row_idx, 2].scatter(corner[0], corner[1], color='green', s=80, marker='o',
                                       edgecolors='white', linewidth=2, alpha=0.8)
                axs[row_idx, 2].text(corner[0] + 5, corner[1] - 5, f'{label}_GT', color='white',
                                    fontsize=8, fontweight='bold',
                                    bbox=dict(boxstyle='round,pad=0.2', facecolor='green', alpha=0.7))
            ...
            for corner_idx, (corner, label) in enumerate(zip(corrected_corners, corner_labels)):
                axs[row_idx, 2].scatter(corner[0], corner[1], color='blue', s=80, marker='s',
                                       edgecolors='white', linewidth=2, alpha=0.8)
                axs[row_idx, 2].text(corner[0] + 5, corner[1] + 10, f'{label}_P', color='white',
                                    fontsize=8, fontweight='bold',
...
def run_roma_tta(image0: torch.Tensor, image1: torch.Tensor, model, device: str, n_augmentations: int = 8, filter_by_certainty: bool = True):
    """
    ...
    for i in range(n_augmentations):
        ...
        with torch.no_grad():
            ...
            warp, certainty = model.match(
                norm_tensor_to_pil(image0_rot), 
                norm_tensor_to_pil(image1), 
                device=device
            )
        ...
        idx0_aug, idx1_aug = model.to_pixel_coordinates(
            matches, 
            image0_rot.shape[1], 
            image0_rot.shape[2], 
            image1.shape[1], 
...
def process_batch(model, batch, true_affine, cfg: DictConfig, device):
    ...
    if cfg.transforms.tf_difficulty is not None and not cfg.transforms.rotate_after:
        ...
        mask = apply_affine_2d_map(torch.ones(1, image1.shape[2], image1.shape[3]), true_affine).to(device)
        mask = torch_erode(mask, kernel_size=3)
    ...
    # If not RoMa
    if not isinstance(model, RegressionMatcher):
        # Get embeddings
        with torch.no_grad():
    ...
    if isinstance(model, RegressionMatcher):
        # Resize for roma
        new_H = 560
        new_W = 560
        image0 = F.interpolate(image0, size=(new_H, new_W), mode='bilinear', align_corners=False)
        image1 = F.interpolate(image1, size=(new_H, new_W), mode='bilinear', align_corners=False)
        ...
        try:
            ...
            if isinstance(model, RegressionMatcher):
                ...
                if cfg.eval.tta_n_augs > 1:
                    ...
                    with torch.no_grad():
                        # Standard matching without TTA
                        warp, certainty = model.match(norm_tensor_to_pil(image0[i]), norm_tensor_to_pil(image1[i]), device=device)
                        # Sample matches for estimation
                        matches, certainty = model.sample(warp, certainty)
                ...
                if cfg.eval.use_gt_correspondances:
                    if has_multiple_augs and not filter_by_certainty:
                        ...
                        if has_multiple_augs:
                            ...
                            print(f"WARNING: Multiple augmentations found but filter_by_certainty is True")
                ...
                idx0, _, idx1, _ = find_nn(e0[i], e1_rot, mask=mask, top_k=100)
            ...
            if cfg.eval.tta_n_augs > 1 and not isinstance(model, RegressionMatcher):
                params_i = {
            ...
            eval_results = evaluate_corner_alignment(
                ...
                img_shape=(cfg.data.image_size[1], cfg.data.image_size[0]), 
...
def main(cfg: DictConfig) -> None:
    ...
    # Load model
    if "roma" in cfg.model.checkpoint:
        ...
        if cfg.model.checkpoint == "roma_outdoor":
            weights_path = os.getenv("ROMA_OUTDOOR_WEIGHTS_PATH", None)
            ...
            model = roma_outdoor(device=device, 
                                    ...
                                    weights=weights,)
        elif cfg.model.checkpoint == "roma_indoor":
            weights_path = os.getenv("ROMA_INDOOR_WEIGHTS_PATH", None)
            weights_path = to_absolute_path(weights_path) if weights_path is not None else None
            weights = torch.load(weights_path, map_location="cpu") if weights_path is not None else None
            ...
            raise ValueError(f"Unsupported RoMa checkpoint: {cfg.model.checkpoint}")
    else:
    ...
    for batch_idx, batch in enumerate(tqdm(val_loader)):
        ...
        batch_results = process_batch(model, batch, true_affine, cfg, device)
        ...
        save_best_worst_alignments(batch, batch_results, batch_idx, cfg)
